        ...
        with torch.no_grad():
            ...
            for adj, mask in [(valid_A, valid_mask), (test_A, test_mask)]:
                if model.attention is None:
                    z = layer2(spmm(adj, H, batch_size))
                else:
                    ...
                    h2 = H2[mask]
                    s = torch.zeros(e, heads).to(H2.device)
                    z = torch.zeros(n, heads).to(H2.device)
                    for cur in erange[::batch_size]:
                    ...
                    z = torch.zeros(n, zj.shape[-1]).to(s.device)
                    for cur in erange[::batch_size]:
                        psrc = src[cur: cur + batch_size]
        ...
        with torch.no_grad():
            ...
            for adj, mask in [(valid_A, valid_mask), (test_A, test_mask)]:
                if model.attention is not None:
                    ...
                    h2 = H2[mask]
                    if model.skip_layer is not None:
                        sk = model.skip_layer(H[mask])
                    src, dst = adj._indices()
                    erange = torch.arange(src.shape[0])
                    v = torch.zeros(src.shape[0], H2.shape[-1]).to(H2.device)
                    w = adj._values()
...
valid_A = valid_A.to(P.device)
test_A = test_A.to(P.device)
